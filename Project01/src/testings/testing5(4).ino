...
unsigned long snooze_until[] = {0, 0}; // Snooze time for each alarm
...
void ring_alarm(int alarm_num) {
    ...
    while (!alarm_acknowledged) {
        ...
        print_line("OK:Stop UP:Snooze", 0, 55, 1);
        display.display();
    ...
    if (snooze_pressed) {
        ...
        unsigned long current_time = hours * 60 + minutes;
        unsigned long snooze_time = current_time + 5;
        
        // Handle overflow (next hour)
        if (snooze_time >= 1440) {
            snooze_time -= 1440;
        }
        
        // Store snooze time
        snooze_until[alarm_num] = snooze_time;
        
        display.clearDisplay();
        print_line("Alarm " + String(alarm_num+1), 0, 0, 1);
        print_line("Snoozed until", 0, 20, 1);
        print_line(String(snooze_time / 60) + ":" + 
                 (snooze_time % 60 < 10 ? "0" : "") + 
                 String(snooze_time % 60), 0, 40, 2);
        delay(2000);
    }
}
...
void update_time_with_check_alarm(void) {
    ...
    if (alarm_enabled == true) {
        for (int i = 0; i < n_alarms; i++) {
            ...
            // Check if alarm should trigger (either regular or snoozed)
            bool should_trigger = false;
            
            // Check regular alarm time
            if (alarm_triggered[i] == false && 
                alarm_hours[i] == hours && 
            ...
            // Check snooze time
            if (snooze_until[i] > 0 && current_time == snooze_until[i]) {
                should_trigger = true;
                snooze_until[i] = 0; // Reset snooze after triggering
            }
    ...
    if (hours == 0 && minutes == 0 && seconds == 0) {
        for (int i = 0; i < n_alarms; i++) {
            alarm_triggered[i] = false;
            snooze_until[i] = 0; // Also clear any snooze times
        }
    }
}
...
void set_alarm(int alarm) {
    ...
    snooze_until[alarm] = 0; // Clear any existing snooze for this alarm
    display.clearDisplay();

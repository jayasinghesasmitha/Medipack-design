...
void send_light_level() {
  ...
  if (currentMillis - lastLdrPublish >= sendInterval && ldrReadingCount >= 10) { // Relaxed condition
    Serial.println("Attempting to publish LDR average...");
    publishLdrAverage();
    lastLdrPublish = currentMillis;
    // Clear oldest readings if buffer exceeds size
...
void control_servo() {
  ...
  float I = ldrReadings.empty() ? 0.0 : ldrReadings.back(); // Latest LDR reading
...
void checkSchedule() {
  if (isScheduleON) {
    unsigned long currentTime = millis() / 1000; // Placeholder

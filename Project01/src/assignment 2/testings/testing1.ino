...
#define PB_DOWN 35
#define DHTPIN 12
#define LDR_PIN 36 // Analog pin A0
#define SERVO_PIN 13
...
#define MQTT_TOPIC_LIGHT "medibox/light_intensity"
#define MQTT_TOPIC_TS "medibox/config/ts"
#define MQTT_TOPIC_TU "medibox/config/tu"
#define MQTT_TOPIC_THETA_OFFSET "medibox/config/theta_offset"
#define MQTT_TOPIC_GAMMA "medibox/config/gamma_factor" // Updated topic
#define MQTT_TOPIC_T_MED "medibox/config/t_med"
...
float gammaFactor = 0.75; // Controlling factor (renamed from gamma)
float t_med = 30.0; // Ideal storage temperature
unsigned long last_sample = 0;
unsigned long last_send = 0;
float light_sum = 0.0;
int sample_count = 0;
const int LDR_MIN = 0; // Adjust based on calibration
const int LDR_MAX = 4095; // ESP32 ADC max value
...
void reconnect_mqtt() {
  while (!mqttClient.connected()) {
    if (mqttClient.connect(MQTT_CLIENT_ID)) {
      ...
      mqttClient.subscribe(MQTT_TOPIC_TU);
      mqttClient.subscribe(MQTT_TOPIC_THETA_OFFSET);
      mqttClient.subscribe(MQTT_TOPIC_GAMMA);
      mqttClient.subscribe(MQTT_TOPIC_T_MED);
    } else {
      delay(5000);
    }
  }
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == MQTT_TOPIC_TS) {
    ts = message.toFloat();
    if (ts < 1) ts = 1; // Minimum 1s
  } else if (String(topic) == MQTT_TOPIC_TU) {
    tu = message.toFloat();
    if (tu < 60) tu = 60; // Minimum 60s
  } else if (String(topic) == MQTT_TOPIC_THETA_OFFSET) {
    theta_offset = message.toFloat();
    if (theta_offset < 0) theta_offset = 0;
    if (theta_offset > 120) theta_offset = 120;
  } else if (String(topic) == MQTT_TOPIC_GAMMA) {
    gammaFactor = message.toFloat();
    if (gammaFactor < 0) gammaFactor = 0;
    if (gammaFactor > 1) gammaFactor = 1;
  } else if (String(topic) == MQTT_TOPIC_T_MED) {
...
void monitor_light() {
  ...
  if (current_time - last_sample >= ts * 1000) {
    int ldr_value = analogRead(LDR_PIN);
    float intensity = (float)(ldr_value - LDR_MIN) / (LDR_MAX - LDR_MIN);
  ...
  // Send average every tu seconds
  if (current_time - last_send >= tu * 1000 && sample_count > 0) {
    float avg_intensity = light_sum / sample_count;
    char buffer[10];
    dtostrf(avg_intensity, 4, 2, buffer);
    mqttClient.publish(MQTT_TOPIC_LIGHT, buffer);

    // Calculate and set servo angle
    TempAndHumidity data = dhtSensor.getTempAndHumidity();
    float T = data.temperature;
    float theta = theta_offset + (180 - theta_offset) * avg_intensity * gammaFactor * log(ts / tu) * T / t_med;
    if (theta < 0) theta = 0;
